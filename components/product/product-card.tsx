...
export default function ProductCard({ product, className }: ProductCardProps) {
  ...
  return (
    <Link
      className={clsx(
        "group relative block h-full xl:h-110 w-full transition-transform duration-150 ease-out hover:-translate-y-1",
        className
      )}
      href={`/product/${product.handle}`}
      prefetch
    >
      <article className="flex h-full flex-col overflow-hidden rounded-lg border border-neutral-800 bg-white pt-5 shadow-[0_25px_80px_rgba(10,10,15,0.45)] transition-colors duration-150 group-hover:border-yellow-500/60 ">
        ...
        <Price
          ...
          />
          <span className="max-w-[100%] flex-1 text-center px-3 py-1 text-md font-extrabold tracking-wide uppercase tracking-[0.25em] text-neutral-900">
            {product.title}
          </span>
        </div>
      </article>
